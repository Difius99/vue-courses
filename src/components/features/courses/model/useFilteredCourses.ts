...
export function useFilteredCourses(
  ...
  const filteredCourses = computed(() => {
    return courses.value.filter((course) => {
      return Object.entries(appliedFilters.value).every(
        ([filterKey, options]) => {
          ...
          return options.some((option) => {
            ...
            if (filterKey === "Стоимость") {
              ...
              const minOk =
                meta?.minCost == null || course.cost >= meta.minCost;
              const maxOk =
                meta?.maxCost == null || course.cost <= meta.maxCost;
              return minOk && maxOk;
            }
            ...
            if (filterKey === "Навыки")
              return course.tags.includes(option.value);
